...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await authService.register({ ...form, plan, billingCycle: isFree ? 'trial' : billing })
      if (isFree) {
        setLicenseKey(res.organization?.licenseKey || res.licenseKey)
      } else {
  ...
  const handleCopy = async () => { if (licenseKey) { await navigator.clipboard.writeText(licenseKey); setCopied(true); toast.success('License key copied!'); setTimeout(() => setCopied(false), 2000) } }
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                <button onClick={handleCopy} className="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">{copied ? <Check size={16} className="text-green-600" /> : <Copy size={16} className="text-gray-500" />}</button>
              </div>
            </div>
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gray-50 dark:bg-gray-900">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <div className="inline-block mt-2 px-3 py-1 bg-primary-50 dark:bg-primary-900/20 rounded-full"><span className="text-sm font-medium text-primary-700 dark:text-primary-400">{planName}</span>{!isFree && <span className="text-sm text-primary-600"> · {billing}</span>}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">I agree to the <button type="button" onClick={() => setLegalModal('terms')} className="text-primary-600 hover:underline">Terms & Conditions</button> and <button type="button" onClick={() => setLegalModal('privacy')} className="text-primary-600 hover:underline">Privacy Policy</button></span>
            </label>
            <Button type="submit" loading={loading} className="w-full" disabled={!agreedToTerms}>{isFree ? 'Start Free Trial' : 'Continue to Payment'}</Button>
          </form>
        </div>
        <p className="text-center text-sm text-gray-500 mt-4">Already have an account? <Link to="/login" className="text-primary-600 hover:underline">Sign in</Link></p>
      </div>

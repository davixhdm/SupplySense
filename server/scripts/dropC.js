...
async function main() {
  ...
  if (collectionNames.length === 0) {
    console.log('\x1b[33m⚠️  No collections found.\x1b[0m\n');
  ...
  console.log(`\x1b[33m  Collections (${collectionNames.length})\x1b[0m`);
  console.log('\x1b[36m═══════════════════════════════════════\x1b[0m\n');

  collectionNames.forEach((name, i) => {
    console.log(`${i + 1}. ${name}`);
  });
  console.log(`${collectionNames.length + 1}. Cancel`);

  const choice = await question('\n\x1b[37mSelect collection to drop: \x1b[0m');
  ...
  const selected = collectionNames[index];

  console.log(`\n\x1b[31m⚠️  You are about to drop: ${selected}\x1b[0m`);
  const confirm = await question(`\x1b[31mType "DROP ${selected}" to confirm: \x1b[0m`);

  if (confirm !== `DROP ${selected}`) {
    console.log('\x1b[33mCancelled.\x1b[0m\n');
    await mongoose.disconnect();
    rl.close();
    return;
  ...
  console.log(`\x1b[32m✅ Collection "${selected}" dropped.\x1b[0m\n`);

...
const customerSchema = new mongoose.Schema(
  {
    ...
    customerType: {
      type: String,
      enum: ['individual', 'business', 'wholesale', 'vip'],
      default: 'individual'
    },
    totalPurchases: {
      type: Number,
...
customerSchema.methods.recordPurchase = async function (amount) {
  ...
  if (!this.firstPurchaseDate) {
    this.firstPurchaseDate = now;
  }
  this.lastPurchaseDate = now;
...
customerSchema.methods.calculateChurnRisk = async function () {
  ...
  const daysSinceLastPurchase = Math.ceil(
    (new Date() - this.lastPurchaseDate) / (1000 * 60 * 60 * 24)
  );
  ...
  if (daysSinceLastPurchase > 180) {
    ...
    this.churnRisk = Math.max(0, Math.floor(daysSinceLastPurchase / 3));
  }

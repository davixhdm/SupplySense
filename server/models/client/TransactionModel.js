...
const transactionSchema = new mongoose.Schema(
  {
    ...
    type: {
      type: String,
      enum: ['sale', 'purchase', 'return', 'refund', 'expense', 'adjustment', 'transfer'],
      required: true
    },
    amount: {
      type: Number,
    ...
    paymentMethod: {
      type: String,
      enum: ['cash', 'card', 'mobile_money', 'bank_transfer', 'credit', 'other'],
      default: 'other'
    },
    status: {
      type: String,
...
transactionSchema.index({ organizationId: 1, customerId: 1 });
transactionSchema.index({ organizationId: 1, productId: 1 });
